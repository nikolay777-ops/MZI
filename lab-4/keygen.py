...
class HammingKeygen:
    def __init__(self, kgen):
        ...
        self.G = self._gen_hamming_matrix()
        self.S = self._gen_invert_matrix()
        self.P = self._gen_permutate_matrix()
        self.Gcarat = np.matmul(np.matmul(self.S, self.G), self.P) % 2

    def _gen_hamming_matrix(self):
        identity = np.identity(self.kgen)
        identityk = np.identity(self.k)
        left = np.zeros((self.kgen, self.n - self.kgen)).T
        rowcount = 0
    ...
    def _gen_permutate_matrix(self):
